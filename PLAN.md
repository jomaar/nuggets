...
| ~~10~~ | ~~icon-192.png / icon-512.png + Favicon~~ | ✅ | **Erledigt 2026-06-10.** Brand-Logo (`assets/nuggets-logo.svg`) per `scripts/generate-icons.mjs` (sharp) rasterisiert → `public/icon-192.png`, `public/icon-512.png` (opak weiß, iOS-tauglich) + `app/favicon.ico` (16/32/48 px, PNG-eingebettet). Logo wird auf weißem Grund getrimmt + zentriert (`LOGO_SCALE` 0.82). Reproduzierbar via `node scripts/generate-icons.mjs`. |
...
| 15 | 💭 UX-Überarbeitung Einzelansicht (Design-Session) | mittel | **User-Wunsch 2026-07-14 („Schritt 2" nach dem „Nur Text"-Toggle): Benutzerführung stringenter, klarer, kreativer, direkter, übersichtlicher.** Ausgangsbefund: Die Bedienung ist historisch gewachsen und auf viele Orte verteilt — Sticky-Bar mit 9–10 Icons; Ansicht-Zeile (Nur Text, Schriftgröße, Versangaben, Kommentarstellen); Info-Panel mit DUPLIKAT-Reglern (Schriftgröße, Versangaben); Markierungen-Popup mit Legende (Namen, Augen-Toggles, Schema-Import) + Sprungliste; Kommentar-Sheet; BubbleMenu (2 Farb-Reihen, Kommentar, ✕). Fragen für die Session: Was gehört zusammen? (z. B. alle Ansichts-/Lese-Einstellungen an EINEN Ort statt Ansicht-Zeile + Info-Panel), Icon-Anzahl reduzieren (Gruppierung? Mehr-Ebenen-Menü? Gesten?), Duplikate abbauen, Konsistenz der Öffnungsmuster (Zeile in Sticky-Bar vs. Popup vs. Bottom-Sheet). **Konkreter User-Input (2026-07-14): Drucken, Löschen und Info werden selten gebraucht → unter einem „…"- oder ⚙️-Sammelbutton zusammenfassen** (entlastet die Sticky-Bar um 2 Icons netto). Erst Design klären (eigene Session, kein Code vorab); danach ggf. in Etappen umsetzen. |
